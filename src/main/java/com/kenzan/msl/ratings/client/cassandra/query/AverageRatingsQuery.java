...
public class AverageRatingsQuery extends RatingsHelper {
  ...
  public static void add(final QueryAccessor queryAccessor, final MappingManager manager,
    ...
    if (isValidContentType(contentType)) {
      queryAccessor.setAverageRating(averageRatingsDto.getContentId(),
          averageRatingsDto.getContentType(), averageRatingsDto.getNumRating(),
          averageRatingsDto.getSumRating());
      // Verifies that average rating was in fact added
      if (!get(queryAccessor, manager, averageRatingsDto.getContentId(),
          averageRatingsDto.getContentType()).isPresent()) {
   ...
   */
  public static Optional<AverageRatingsDto> get(final QueryAccessor queryAccessor,
      final MappingManager manager, final UUID contentId, final String contentType) {
   ...
   * @param contentType String
   */
  public static void delete(final QueryAccessor queryAccessor, final MappingManager manager,
    ...
    if (isValidContentType(contentType)) {
      ...
      if (get(queryAccessor, manager, contentId, contentType).isPresent()) {
        throw new RuntimeException(String.format(
            "Unable to remove from average_ratings, contentId: %s", contentId));
      }
    } else {
      throw new RuntimeException(String.format("Invalid contentType: %s", contentType));
    }
