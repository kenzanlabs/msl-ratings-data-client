...
public interface QueryAccessor {
    ...
    public void setAverageRating(@Param("content_id") UUID content_id, @Param("content_type") String content_type,
                                 @Param("num_rating") long num_rating, @Param("sum_rating") long sum_rating);

    @Query("SELECT * FROM average_ratings WHERE content_id = :content_id AND content_type = :content_type LIMIT 1")
    public ResultSet getAverageRating(@Param("content_id") UUID content_id, @Param("content_type") String content_type);

    @Query("DELETE FROM average_ratings WHERE content_id = :content_id AND content_type = :content_type")
    public void deleteAverageRating(@Param("content_id") UUID content_id, @Param("content_type") String content_type);
