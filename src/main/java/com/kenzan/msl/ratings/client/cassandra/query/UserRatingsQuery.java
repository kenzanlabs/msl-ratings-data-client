...
public class UserRatingsQuery extends RatingsHelper {
  ...
  public static void add(final QueryAccessor queryAccessor, final MappingManager manager,
    ...
    if (isValidContentType(contentType)) {
      queryAccessor.setUserRating(userRatingsDto.getUserId(), userRatingsDto.getContentUuid(),
          contentType, userRatingsDto.getRating());
      // Verifies that user rating was in fact added
      if (!getRating(queryAccessor, manager, userRatingsDto.getUserId(),
          userRatingsDto.getContentUuid(), contentType).isPresent()) {
        throw new RuntimeException(String.format(
            "Unable to add to user_ratings, contentId: %s, userId: %s",
   ...
   */
  public static Optional<UserRatingsDto> getRating(final QueryAccessor queryAccessor,
      final MappingManager manager, final UUID userId, final UUID contentId,
   ...
   * @param contentType String
   */
  public static void remove(final QueryAccessor queryAccessor, final MappingManager manager,
    ...
    if (isValidContentType(contentType)) {
      ...
      if (getRating(queryAccessor, manager, userId, contentId, contentType).isPresent()) {
        throw new RuntimeException(String.format(
            "Unable to remove contentId: %s from user_ratings table", contentId));
      }
    } else {
      throw new RuntimeException(String.format("Invalid contentType: %s", contentType));
    }
